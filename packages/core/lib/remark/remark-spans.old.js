...
function md2html () {
  return function transformer (tree, file) {
    visit(tree, 'code', function visitor (node, i, parent) {
      ...
      if (data) {
        ...
        // console.log('test', parent, node, parent.children[i])
        // parent.children.splice(i, 1, {type: 'html', children: data.html})
        // function modifier(node, index, parent) {
        //    parent.children.splice(index, 1, {type: 'html', children: data.html})
        //   return index + 1
        // }
        ...
        let definition = [{
          ...
          data: {
            hName: 'span',
            hChildren: [{type: 'text', value: data.text}]
          }
        }];
...
function parseMarkdown (node, i, parent, tree) {
  ...
  //console.log('', parent.children[i + 1].value)

  if (node.type &&
    node.type === 'linkReference' &&
    node.children &&
    parent.children[i + 1].value &&
    parent.children[i + 1].value.match(/\{(.+)\}/) &&
